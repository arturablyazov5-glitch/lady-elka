...
function renderSettings(){if(!catalogSettings||changed)return;state=structuredClone(catalogSettings);for(const id of ['price-adjustment','site-discount','new-type','reset-settings'])$('#'+id).disabled=false;$('#price-adjustment').value=state.adjustment_pct;$('#site-discount').value=state.discount_pct;renderTypes();preview();if(!$('#settings-integration-link'))$('#settings-form .settings-save').insertAdjacentHTML('beforebegin','<a class="settings-link-card" id="settings-integration-link" href="#integration"><span class="settings-symbol" data-icon="globe"></span><span><strong>Подключение сайта</strong><small>Ссылки на CSV для размещения каталога в Taptop</small></span><span class="settings-link-arrow" aria-hidden="true">›</span></a>');icons($('#settings-integration-link'));$('#save-settings').disabled=true;message('Все изменения сохранены');}
