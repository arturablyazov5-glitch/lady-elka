...
function createTabComponent(root,{ariaLabel,items,selected,onChange}){
 ...
 render();return{setItems(next,selectedValue){items=next;if(selectedValue!==undefined)current=selectedValue;render();},select,get value(){return current;}};
}
...
// При локальной разработке кабинет и API обслуживает один сервер на :4180.
// В опубликованной версии сохраняем адрес Cloud Function, чтобы не менять
// существующее подключение без отдельного развёртывания.
const isLocalHost=['localhost','127.0.0.1','::1'].includes(location.hostname);
const API_BASE=isLocalHost?location.origin:'https://mgnotvaahftrbifqtahf.supabase.co/functions/v1/catalog';
...
function joinControls(pageId,tabsSelector='.catalog-toolbar'){const page=$(pageId),tabs=page?.querySelector(tabsSelector),filters=page?.querySelector('.filter-bar');if(!tabs||!filters)return;if(tabsSelector==='.catalog-toolbar'){if(!tabs.contains(filters)){tabs.classList.add('controls-panel');tabs.append(filters);}}else if(!tabs.parentElement.classList.contains('controls-panel')){const panel=document.createElement('div');panel.className='controls-panel';tabs.parentElement.insertBefore(panel,tabs);panel.append(tabs,filters);}}
joinControls('#catalog-page');joinControls('#promos-page');
...
function render(){
 ...
 $('#product-list').innerHTML=list.map(p=>{
  ...
  return `<article class="product-row" data-edit="${esc(p.id)}" role="button" tabindex="0" aria-label="Открыть ${esc(p.title)}"><div class="product-photo">${photo?`<img src="${esc(photo)}" alt="${esc(p.title)}" loading="lazy" referrerpolicy="no-referrer">`:pict('image')}</div><div class="product-summary"><div class="product-title-row"><h3>${esc(p.title)}</h3><span class="product-variant-count">Вариантов: ${p.variants.length}</span></div><div class="product-meta"><span class="product-sku">Артикул ${esc(p.sku)}</span></div><div class="product-category"><span class="product-section-label">${esc(sectionName(p))}</span>${categories.map(category=>`<span class="product-category-chip">${esc(category)}</span>`).join('')}</div></div><div class="product-price">${p.variants.length>1?'от ':''}${rub(price)}<span class="visibility-state ${active.length?'':'off'}"><span></span>${active.length?'На сайте':'Скрыт'}</span>${active.length&&active.length<p.variants.length?`<small>${active.length} из ${p.variants.length} вариантов</small>`:''}</div><button class="icon-btn row-edit" type="button" data-edit="${esc(p.id)}" aria-label="Редактировать ${esc(p.title)}">${pict('pencil')}</button></article>`;
 }).join('');
...
function navigate(page){const current=['catalog','orders','promos','analytics','integration','settings','payment','profile','profile-payments','profile-history'].includes(page)?page:'catalog';for(const key of ['catalog','orders','promos','analytics','integration','settings','payment','profile','profile-payments','profile-history']){const el=$(`#${key}-page`);if(el){el.hidden=current!==key;el.classList.toggle('active',current===key);}}$$('.side-tabs [data-nav]').forEach(a=>{if(a.dataset.nav===(current.startsWith('profile')?'profile':current))a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});document.body.classList.toggle('payment-active',current==='payment');document.body.classList.toggle('profile-active',current.startsWith('profile'));document.querySelectorAll('.top-tabs [data-nav]').forEach(a=>{const selected=a.dataset.nav===(current==='payment'?'payment':'catalog');a.classList.toggle('active',selected);if(selected)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});document.title=`${{profile:'Профиль','profile-payments':'Способы оплаты','profile-history':'История операций',payment:'Оплата',analytics:'Аналитика',catalog:'Каталог',orders:'Заказы',promos:'Промокоды',integration:'Подключение сайта',settings:'Настройки'}[current]} — Lady Elka`;window.dispatchEvent(new CustomEvent('page-shown',{detail:current}));}
window.addEventListener('hashchange',()=>{navigate(location.hash.slice(1));window.scrollTo(0,0);});navigate(location.hash.slice(1));
function syncSettingsNav(page){const active=page==='integration'?'settings':page;$$('.side-tabs [data-nav]').forEach(a=>{if(a.dataset.nav===active)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});}
window.addEventListener('page-shown',e=>{if(e.detail==='integration')syncSettingsNav(e.detail);});syncSettingsNav(location.hash.slice(1));
...
function renderFeeds(){
 const page=$('#integration-page');if(page&&!page.querySelector('[data-back-settings]'))page.insertAdjacentHTML('afterbegin','<a class="neo-back-button" data-back-settings href="#settings">← Настройки</a>');
