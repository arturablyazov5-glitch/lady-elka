...
window.openEditor=function(product,seed=null){
 ...
 form.elements.title.value=draft.title;form.elements.sku.value=draft.sku;
...
form.elements.kind.addEventListener('change',e=>{const selected=e.target.value;draft.kind=selected==='decor'?'decor':'trees';if(selected==='trees'||selected==='thuja'){const type=catalogSettings.types.find(t=>t.id===(selected==='thuja'?'thuja':'green'))||catalogSettings.types[0];draft.variants.forEach(v=>{v.type_id=type.id;v.category=type.name;});}draft.id=`${draft.kind}:${draft.sku}`;activeGroup=getGroups(draft)[0];dirty();renderGroups();});
...
$('#variants').addEventListener('click',e=>{
 ...
 if(b.dataset.photoDownload!==undefined){downloadPhoto(contentRows()[0].photos[+b.dataset.photoDownload],+b.dataset.photoDownload);return;}
...
function photoFileName(url,index){
 try{const name=decodeURIComponent(new URL(url,location.href).pathname.split('/').pop()||'');if(name&&/\.[a-z0-9]{2,5}$/i.test(name))return name.replace(/[^\w.\-]+/g,'_');}catch{}
 return `lady-elka-photo-${index+1}.webp`;
}
async function downloadPhoto(url,index=0){
 if(!url)return;
 const button=$('#photo-download');if(button)button.disabled=true;
 try{const response=await fetch(url,{mode:'cors'});if(!response.ok)throw new Error('download failed');const blob=await response.blob(),objectUrl=URL.createObjectURL(blob),link=document.createElement('a');link.href=objectUrl;link.download=photoFileName(url,index);link.rel='noopener';document.body.append(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(objectUrl),1000);}
 catch{const link=document.createElement('a');link.href=url;link.target='_blank';link.rel='noopener';link.click();toast('Фото открыто в новой вкладке — сохраните его через меню браузера');}
...
$('#photo-download').addEventListener('click',()=>downloadPhoto(lightboxPhotos[lightboxIndex],lightboxIndex));
