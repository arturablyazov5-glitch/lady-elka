// === НАСТРОЙКА
const SHEET_CSV_URL='https://docs.google.com/spreadsheets/d/e/2PACX-1vQBYDczXmGsnVqSrCuzjsMwz6-DZu3Q6pSjb66YUkgPxxt7UDecZLll9QZFHU0BHhKc3GMZ4xFoouXB/pub?gid=0&single=true&output=csv';
const DECOR_SHEET_CSV_URL='https://docs.google.com/spreadsheets/d/e/2PACX-1vQBYDczXmGsnVqSrCuzjsMwz6-DZu3Q6pSjb66YUkgPxxt7UDecZLll9QZFHU0BHhKc3GMZ4xFoouXB/pub?gid=500105078&single=true&output=csv';

// === УТИЛЫ
const $$=s=>Array.from(document.querySelectorAll(s));
const num=s=>Number(String(s||'').replace(/[^\d.-]/g,''));           // "22 000" -> 22000
const rub=n=>(Number(n)||0).toLocaleString('ru-RU')+' ₽';
const norm=s=>String(s||'').trim().toLowerCase().replace(/[ёЁ]/g,'е').replace(/\s+/g,' ');
// Делаем доступной глобально для скрипта промокодов
if (typeof window !== 'undefined') window.norm = norm;
// &#x2B07;️ сюда добавь (глобально, чтобы видели все скрипты)
const normCat = s => norm(String(s||'')
  .replace(/&quot;|&#34;/gi, '"')
  .replace(/^"(.*)"$/, '$1')
);
function parseCSV(txt){const o=[],r=[];let i=0,c="",q=false;while(i<txt.length){const ch=txt[i];
...
// положи рядом с утилитами сверху
...
function haveBuilderFields(form){
...
function fillBuilderFields(form, item){
  const set = (label, val) => {
...
function fillBuilderFieldsMultiple(form, cart){
...
// карточка + селекторы фото
const CARD_SEL = '.product-card, .product-wrapper__cms';

// обёртки главных картинок (по категориям)
const IMG_WRAP_SEL = '.product__img, .product__img__cms';

// обёртки «Ещё фото» (под заголовком)
const EXTRA_IMG_WRAP_SEL = '.product__img-dop, .product__img-dop__cms';
